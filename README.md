...
`ansv-haptics-sim` is a lightweight, zero-dependency 2nd-order harmonic oscillator solver for educational demonstration and haptic actuator physics modeling.

> 💡 **Commercial Multi-Physics Digital Twin Simulation Hub**:  
> This open-source repository demonstrates basic linear 2nd-order differential equations. For advanced non-linear stiffness $K(x)$ modeling, dynamic damping $C(v)$, electro-thermal drift, and custom active braking algorithms, please visit our official simulator: [https://ansv.net/simulations/](https://ansv.net/simulations/)
