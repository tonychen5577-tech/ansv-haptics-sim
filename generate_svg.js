...
function generateSVGAnimation(data, params) {
  ...
  <polyline points="${points}" fill="none" stroke="url(#lineGrad)" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" filter="url(#glow)">
    <animate attributeName="stroke-dasharray" from="0 5000" to="5000 0" dur="4.5s" repeatCount="indefinite" />
  </polyline>

  <!-- 扫描前沿发光粒子 -->
  <circle r="6" fill="#00f2fe" filter="url(#glow)"><animateMotion path="M ${points}" dur="4.5s" repeatCount="indefinite" /></circle>
  <circle r="2.5" fill="#ffffff"><animateMotion path="M ${points}" dur="4.5s" repeatCount="indefinite" /></circle>
