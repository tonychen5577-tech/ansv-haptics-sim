...
// Test Case 2: Damped Decay Physical Law Check
{
  const res = solveBasicDampedOscillator({ damping: 0.03 }, 0.1, 0.0001);
  const disp = res.displacement;
  
  let peakValues = [];
  for (let i = 1; i < disp.length - 1; i++) {
    if (disp[i] >= disp[i - 1] && disp[i] >= disp[i + 1] && disp[i] > 0) {
      if (peakValues.length === 0 || Math.abs(disp[i] - peakValues[peakValues.length - 1]) > 0.00001) {
  ...
  assert.ok(peakValues[0] > peakValues[1], 'Peak 1 must be strictly greater than Peak 2 under physical damping');
  console.log(`✅ Test 2 Passed: Physical damping verified (Peak 1: ${(peakValues[0]*1000).toFixed(3)}mm > Peak 2: ${(peakValues[1]*1000).toFixed(3)}mm).`);
}
